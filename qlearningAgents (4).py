...
class QLearningAgent(ReinforcementAgent):
    ...
    def computeValueFromQValues(self, state):
        ...
        """
        "*** YOUR CODE HERE ***"
        #We get all possible actions for the given state
        actions = self.getLegalActions(state)
        #If there the no possible actions from the given state (Terminal state)
        if len(actions) == 0:
          return 0
        else:
          #We get the max of the QValues of all actions for the current state
    ...
    def computeActionFromQValues(self, state):
        ...
        if len(actions) == 0:
          ...
          #We add all the actions to the list if the QValue of the action for the state is equal to the computed Max QValue of the state
          maxAction = [action for action in actions if self.getQValue(state, action) == stateMaxQValue]
          #Among the list of best actions, we select the final action randomly 
